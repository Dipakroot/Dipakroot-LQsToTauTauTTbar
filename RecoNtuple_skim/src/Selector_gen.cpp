...
void Selector_gen::filter_taus(){
  for(int tauInd = 0; tauInd < tree->nMC_; ++tauInd){
    if  (TMath::Abs(tree->mcPID->at(tauInd))==15){
      ...
      bool passTight = (pt > tau_Pt_cut &&
			TMath::Abs(eta) < tau_Eta_tight);
...
void Selector_gen::filter_jets(){
	...
	for(int jetInd = 0; jetInd < tree->jetGenJetPt_->size(); ++jetInd){
		...
		for(std::vector<int>::const_iterator tauInd = Taus.begin(); tauInd != Taus.end(); tauInd++) {
		  if (dR(eta, phi, tree->mcEta->at(*tauInd), tree->mcPhi->at(*tauInd)) < veto_lep_jet_dR) passDR_lep_jet = false;
                }
